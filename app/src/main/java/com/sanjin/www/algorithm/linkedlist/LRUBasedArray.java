package com.sanjin.www.algorithm.linkedlist;
 ...
 *
 * note：
 * 本类的实现中，是将新数据存在头部的，这样就涉及到数据移动，比较浪费内存，我们可以优化一下，
 * 改为把新数据存在尾部，这样就不用搬移数据了。
 * 但是中间部分数据要移到后面的时候，还是会涉及到数据搬移。我们可以模仿SparseArray，先做标记，等达到一定量后再集中处理。
 *
 */
